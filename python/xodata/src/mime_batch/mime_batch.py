...
class BatchRequest:
    ...
    def append(self, part: PartRequest, source: Any = None) -> None:
        if self.part_limit is not None and self.part_count + 1 > self.part_limit:
            raise BatchError(f"Batch request part limit exceeded: limit={self.part_limit}")
    ...
    def add_stats(self, to: BatchRequest, auto_print_stats_interval: int = 0) -> None:
        to.part_limit = max(to.part_limit or 0, self.part_limit or 0)
...
class BatchResponse:
    ...
    def append(self, part: PartResponse) -> None:
        if self.part_limit is not None and self.part_count + 1 > self.part_limit:
            raise BatchError(f"Batch response part limit exceeded: limit={self.part_limit}")
        self.parts.append(part)
