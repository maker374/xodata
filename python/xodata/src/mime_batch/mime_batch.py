...
def post_batch_request(
	...
	batch_url = base_url.rstrip("/") + "/$batch"
	headers = {
	...
	if not content_type:
		raise ValueError("Batch response missing Content-Type header")
	...
	if check_response_part_count and len(parsed.parts) != len(request.parts):
		raise ValueError(
			f"Batch response part count mismatch: expected {len(request.parts)}, got {len(parsed.parts)}"
		)
	return parsed
